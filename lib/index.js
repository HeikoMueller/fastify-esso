...
/** Pull request: Replaced static secret with dynamic array of secrets */
...
module.exports = function builder(opts = default_opts) {
  ...
  async function plugin(fastify, options, done) {
    ...
    async function validation(req, reply) {
      ...
      /** @type { string } */
      let field = null;
      fastify.log.warn("HEADER ?? %s", req.headers[opts.header_name])
      fastify.log.warn("COOKIE ?? %o", req.cookies)
      ...
      /** @type { string } */
      let token;
      fastify.log.warn("TRY VALIDATION WITH %s : %s", key, token)
